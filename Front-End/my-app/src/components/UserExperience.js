  ...
  const getData = async () => {
    try {
      ...
      const resSatisfaction = await fetch(
        `${SERVER_URL}/${item.id}/satisfactions/${item.SatisfactionId}`
      );
      const dataSatisfaction = await resSatisfaction.json();

      const resAgglomeration = await fetch(
        `${SERVER_URL}/${item.id}/agglomeration/${item.AgglomerationId}`
      );
      const dataAgglomeration = await resAgglomeration.json();

      const resTransport = await fetch(
        `${SERVER_URL}/${item.id}/transportBy/${item.TransportById}`
      );
      ...
      const formattedDate =
        date.substring(0, 10) + "T" + date.substring(11, 16);
    ...
    <div>
      <div className="card">
        <div className="image-content">
          ...
          <div className="card-image">{renderSwitch()}</div>
          <div className="card-content">
            ...
            <div className="card-row">
              <label className="card-label">Departure:</label>
              <input
            ...
            <div className="card-row">
              <label className="card-label">Arrival:</label>
              <input
                className="card-input"
                type="text"
                value={arrival}
            ...
            <div className="card-row">
              <label className="card-label">Time(mins):</label>
              <input
                className="card-input"
                type="number"
                value={duration}
            ...
            <div className="card-row">
              <label className="card-label">Traffic:</label>
              <div>
                <select
                  className="dropdown"
                  value={trafficOption}
            ...
            <div className="card-row">
              <label className="card-label">Vehicle:</label>
              <div>
                <select
                  className="dropdown"
                  value={vehicleOption}
            ...
            <div className="card-row">
              <label className="card-label">Satisfaction:</label>
              <div>
                <select
                  className="dropdown"
                  value={satisfactionOption}
                  onChange={handleSatisfaction}
                >
                  {satisfactionArray.map((s) => (
                    <option key={s.id} value={s.level}>
                      {s.level}
                    </option>
                  ))}
                </select>
            ...
            <div className="card-row">
              <label className="card-label">Share:</label>
              <input
                type="checkbox"
